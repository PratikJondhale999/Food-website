...
app.use(session({
  secret: "pratik@139eu2", //Replace with u r secret key
  resave: false,
...
app.get('/', async (req, res) => {
  const allfood = await fooditem.find({});
    ...
    try {
      const foodItems = await fooditem.find().skip(skip).limit(limit).exec(); //'limit' restricts the number of items to be fetched
      const count = await fooditem.countDocuments();
      const allfood = foodItems;  // Store the fetched food items in 'allfood'
